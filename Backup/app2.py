...
ASSETS_DIR = "assets"
PROJECTPLAN_IMG = os.path.join(ASSETS_DIR, "projektplan.png")          # Screenshot/Export reinlegen
AENDERUNG_TEMPLATE_IMG = os.path.join(ASSETS_DIR, "aenderung_template.png")  # optionales Templatebild

# ---------------------------------------------------------
# GLOBAL CSS (webbiger + Sticky LOP)
# ---------------------------------------------------------
st.markdown(
    """
<style>
...
def seed_data():
    ...
    # Änderungsliste (Demo): Option soll Link-Look haben
    ...
    # LOP (Demo – breite Tabelle + sticky first col)
    lop = pd.DataFrame(
        [
            {
...
st.session_state.setdefault("drive_path", [])  # breadcrumb list
...
def header_bar(title: str):
    ...
    with c2:
        st.markdown(
            "<div style='text-align:right; font-weight:700; padding-top:18px;'>Sonderwunsch</div>",
            unsafe_allow_html=True,
        )
    st.divider()
...
# ---------------------------------------------------------
# PAGE: Änderungsliste (Option=Hyperlinks, rechts Preview)
# ---------------------------------------------------------
def render_change_table_with_links(df: pd.DataFrame):
    rows_html = ""
    for _, r in df.iterrows():
        option = html.escape(str(r["Option"]))
        # Klick auf Link setzt query param -> wir nutzen das nur als "Signal"
        rows_html += f"""
        <tr>
          <td style="padding:10px; border-bottom:1px solid #E6E6E6;">
            <a class="demo-link" href="?opt={option}">{option}</a>
          </td>
          <td style="padding:10px; border-bottom:1px solid #E6E6E6;">{html.escape(str(r["Beschreibung"]))}</td>
          <td style="padding:10px; border-bottom:1px solid #E6E6E6;">{html.escape(str(r["Kategorie"]))}</td>
          <td style="padding:10px; border-bottom:1px solid #E6E6E6;">{html.escape(str(r["Status"]))}</td>
          <td style="padding:10px; border-bottom:1px solid #E6E6E6;">{html.escape(str(r["Wann zuletzt geändert"]))}</td>
        </tr>
        """
    ...
    <div class="demo-card">
      <table style="width:100%; border-collapse: collapse;">
        <thead>
          <tr>
            <th style="text-align:left; border-bottom:1px solid #E6E6E6; padding:10px;">Option</th>
            <th style="text-align:left; border-bottom:1px solid #E6E6E6; padding:10px;">Beschreibung</th>
            <th style="text-align:left; border-bottom:1px solid #E6E6E6; padding:10px;">Kategorie</th>
            <th style="text-align:left; border-bottom:1px solid #E6E6E6; padding:10px;">Status</th>
            <th style="text-align:left; border-bottom:1px solid #E6E6E6; padding:10px;">Wann zuletzt geändert?</th>
          </tr>
        </thead>
        <tbody>
          {rows_html}
...
def page_change_list():
    ...
    # Link-Click signal über Query Param
    qp = st.query_params
    if "opt" in qp:
    ...
    with left:
        ...
        q = st.text_input("Suche", placeholder="z. B. OE3-001 oder Text…")
    ...
    with mid:
        ...
        render_change_table_with_links(show_df)

        st.caption("Klick auf einen Link in **Option** → rechts unten erscheint immer dieselbe Vorschau (Demo).")
    ...
    with right:
        ...
        if st.session_state.change_preview_open:
            ...
            st.markdown('<div class="preview-sub">Leeres Blatt (Demo) – egal welcher Option-Link.</div>', unsafe_allow_html=True)
            ...
            if st.button("Vorschau schließen", use_container_width=True):
                ...
                # Query param entfernen (damit beim Reload nicht wieder aufspringt)
                try:
            ...
            st.caption("Klicke links in der Tabelle auf einen Option-Link → Vorschau erscheint hier.")


# ---------------------------------------------------------
# PAGE: Kalkulationsvorlage (editierbar + leere Spalte + neue Zeilen)
# ---------------------------------------------------------
def page_costing():
    header_bar("Kalkulationsvorlage")
    st.info(
        "Tabelle ist editierbar + **zusätzliche leere Spalte** zum Ausfüllen. "
        "Neue Zeilen sind möglich (Demo)."
    )
    ...
    base_cols = [
        ...
        "FTE indirekt (PAG)",
        "Eingabe (leer)",  # zusätzliche leere Spalte
    ]
...
# ---------------------------------------------------------
# PAGE: LOP (sticky first col + horizontal scroll)
# ---------------------------------------------------------
...
def page_lop():
    header_bar("LOP")
    st.info(
        "Wichtig: horizontales Scrollen (viele Spalten). "
        "Beim Rüberscrollen bleibt die erste Spalte **Arbeitsaufgabe** stehen."
    )

    # Demo-Daten: ein paar erfundene Zeilen, ca. 10
    df = st.session_state.lop.copy()
    # auf aktuelles Projekt filtern (optional)
    # Für Demo lassen wir’s drin, du kannst es aber so filtern:
    # df = df[df["Wichtige Projekte"] == st.session_state.selected_project].copy()

    # für Demo einfach auf 10 Zeilen aufblasen
    while len(df) < 10:
...
# ---------------------------------------------------------
# PAGE: Laufwerk (Ordnerstruktur klickbar + Back)
# ---------------------------------------------------------
def page_drive():
    header_bar("Laufwerk")
    st.info("Ordnerstruktur nachbauen: in Ordner reinklicken, leeren Ordner sehen, zurück gehen reicht für die Demo.")
    ...
    with c1:
        if st.button("⬅️ Back", use_container_width=True, disabled=(len(st.session_state.drive_path) == 0)):
...
# ---------------------------------------------------------
# Dummy pages: Teams / Organigramm / CPM
# ---------------------------------------------------------
def page_teams():
    header_bar("Teams")
    st.info("Demo: hier später Link zur Teams/SharePoint Ordnerstruktur. Für jetzt reicht ein Platzhalter.")


def page_org():
    header_bar("Projektorganigramm")
    st.info("Demo: Organigramm als Bild/Tabelle. Für jetzt reicht ein Platzhalter.")


def page_cpm():
    header_bar("CPM")
    st.info("Demo: Link/Einbettung zu CPM (SAP PT1). Für jetzt reicht ein Platzhalter.")
...
# python3 -m streamlit run Lion/app2.py